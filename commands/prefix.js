exports.run = (bot, msg, args, db, roles) => {
    ...
    var cmdprefix = db.get(`servers.${msg.guild.id}.prefix`).value();
    ...
    if (args[0] == undefined) {
        ...
        if (args[0] == 'default') {
            ...
            db.set(`servers.${msg.guild.id}.prefix`, `${settings.default_prefix}`).write();
        ...
        console.log(`${msg.author.username} (tag: ${msg.author.tag}) on ${msg.guild.name} attempted to use prefix cmd without sufficient permisison level.`);
        return;
    }
    var prefix = args[0].trim();

    //Sets the new prefix to the server id.
    db.set(`servers.${msg.guild.id}.prefix`, `${prefix}`).write();
